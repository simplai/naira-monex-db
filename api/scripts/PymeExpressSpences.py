...
class PymeExpressSpence:

    def __init__(self, cap, id_naira, anual_sales, net_utility_ltm, sales_ltm, cash, ebitda_ltm,
        operation_flow, total_assets, stockholders_equity, max_bureau_line, credit_balance_active_bureau, anual_inferred_income,
        total_liabilities, warranty_type, warranty_value,
    ...
    def execute(self):
        ...
        # line_proposal_final
        self.line_proposal = min(self.line_score, self.product_limit)
        ...
        elif self.line_proposal == self.product_limit:
            self.line_proposal_final = "línea producto"


        self.leverage = (self.credit_balance_active_bureau + self.cap.requested_amount)/(self.anual_inferred_income/12) if self.anual_inferred_income != 0 else None
        ...
        self.line_proposal_max_bureau = self.cap.requested_amount / self.max_bureau_line if self.max_bureau_line is not None else 0
        ...
        elif self.economic_sector_adjustment == "No Acreditable":
            self.score_economic_sector = "-"
        else:
            self.score_economic_sector = "="
        ...
        # score_liquid_pledge
        if self.liquid_pledge == "Sí" and self.observation_keys == 3:
            self.score_liquid_pledge = "?"
        elif self.liquid_pledge == "Sí":
            self.score_liquid_pledge = "+"
        else:
            self.score_liquid_pledge = "="
        ...
        logger.logTo(self.id_naira, f'Finalizando procesamiento de cálculo de PYME Express', self.__class__.__name__)

    def line_calculation(self, factor):
        self.line_granted = min(factor * self.line_proposal, self.product_limit)
